...
class FuseModelImg(nn.Module):
    def __init__(self, debug_flag=0, sequence=1, stereo=False, feature_win=32, sim=0,
        ...
        '''
        super(FuseModelImg, self).__init__()
        ...
        if fuse_method.startswith('vis_'):  # 'LSTM3D_2, LSTM_conv_2, LSTM_LSTM_2, conv_LSTM_2, conv_conv_2'
            ...
            self.FuseNet = VisibilityFusion(input_dim, hidden_dim, kernel_size=(3, 3), num_layers=2,
                                            seq_output_dim=hidden_dim,
    ...
    def forward(self, sat_map, left_camera_k, right_camera_k, grd_img_left, grd_img_right, loc_shift_left,
        ...
        # projection ground images to overhead view
        grd_feature = None
        if grd_img_left != None:

            grd_imgs = self.Merge_multi_grd2sat \
                (loc_shift_left, loc_shift_right, grd_img_left, None, left_camera_k, right_camera_k,
                 heading, 512)  # [B,S,E,C,H,W], E=1

            B, S, _, C_in, H_in, W_in = grd_imgs.size()
            grd_feature = self.GrdFeatureNet(grd_imgs.view(-1, C_in, H_in, W_in))
            _, C, H_g, W_g = grd_feature.size()
            grd_feature = grd_feature.view(B, S, C, H_g, W_g)

            grd_feature = self.SequenceFusion(grd_feature[:, :, None, :, :, :], attn_pdrop, resid_pdrop, pe_pdrop)  # [B,C,H,W]
